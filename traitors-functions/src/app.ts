...
const mapVote = (v: Vote): Api.components["schemas"]["Vote"] => {
  ...
  // Combining episode (max ~12), round (max ~20), voterId (max ~100),
  // targetId (max ~100)
  // This is a simple hash for display purposes to satisfy the schema contract
  // Algorithm:
  // (series * 1000000) + (episode * 10000) + (voterId * 100) + targetId
  // Ensure we don't overflow standard integer limits
  const id = (v.series * 10000000) + (v.episode * 100000) +
    (v.voterId * 100) + (v.targetId % 100);
...
apiRouter.get("/series/:seriesId/votes",
...
apiRouter.get("/series/:seriesId/candidates",
  async (req: Request, res: Response) => {
    try {
      const seriesId = parseInt(req.params.seriesId as string, 10);
      if (isNaN(seriesId)) {
      ...
      // If extra query params are passed they are usually ignored,
      // but here strict validation is good.
      // However, to be safe and compatible,
      // we'll just respect the ones we know.
      // If strict validation is needed:
      // return res.status(400).send({error: "Invalid sort field"});
      }

      const candidatesDomain = await getCandidatesBySeriesNumber(
