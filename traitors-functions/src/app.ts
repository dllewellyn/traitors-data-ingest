...
const mapVote = (v: Vote): Api.components["schemas"]["Vote"] => {
  ...
  // Algorithm: (series * 1000000) + (episode * 10000) + ...
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (sortBy !== "name") {
        ...
        // However, to be safe and compatible, we'll just respect the ones
        // we know.
      ...
      logger.error(
        `Error getting candidates for series ${req.params.seriesId}`,
        err
      );
      res.status(500).send({error: "Internal Server Error"});
    }
