...
export const getSeriesByNumber = async (
  ...
  try {
    ...
    logger.error({
      message: `Error getting series ${seriesNumber}`,
      error: err,
    });
    throw err;
  }
...
export const getAllSeries = async (): Promise<Series[]> => {
  try {
    ...
    const snapshot = await db.collection("series")
      .orderBy("seriesNumber").get();

    return snapshot.docs.map((doc) => doc.data() as Series);
  } catch (err) {
    logger.error({message: "Error getting all series", error: err});
    throw err;
  }
};
...
export const getCandidatesBySeriesNumber = async (
  ...
  try {
    ...
    logger.error({
      message: `Error getting candidates for series ${seriesNumber}`,
      error: err,
    });
    throw err;
  }
...
export const getVotes = async (
  ...
  try {
    ...
    logger.error({
      message: `Error getting votes for series ${seriesNumber}`,
      error: err,
    });
    throw err;
  }
