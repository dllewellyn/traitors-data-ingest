...
// Note: In a real CI environment, we would start the emulator programmatically or via a service.
// For this 'manual' verification phase, we skip it if the emulator isn't reachable.
...
describe('Emulator Integration Tests', () => {
  ...
  beforeAll(async () => {
    try {
      const response = await request(EMULATOR_HOST).get('/api/health').timeout(1000);
  ...
  it('should support manual trigger on emulator', async () => {
    ...
    expect(response.status).toBe(200);
    expect(response.body).toEqual({ status: 'ingestion started', mode: 'manual' });
  });
});
