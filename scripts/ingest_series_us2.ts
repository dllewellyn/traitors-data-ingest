...
/**
 * Main function to run the data ingestion process for US Series 2.
 */
async function main() {
  try {
    initializeFirebase();
    ...
    console.log("Starting US Series 2 ingestion process...");
    await runIngestionProcess({
      storageWriter: writer,
      seriesIds: ["TRAITORS_US_S2"],
    });

    console.log("US Series 2 ingestion process completed successfully.");
    process.exit(0);
  } catch (error) {
